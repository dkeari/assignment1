...
public class Logic 
    ...
    public void process() {
        ...
        switch (shapeForCalculations) {
            ...
            case Sphere:
                ...
                // two sets of print statements above and the two beow until the 'break;'
            case Circle:
 ...
 public static double boxSurfaceArea(double mLength, double mWidth, double mHeight){
        double area = 2.0 * mWidth * mLength + 2.0 * mHeight * mLength + 2.0 * mHeight * mWidth;
        return (area);
 }
 public static double rectanglePerimeter(double mWidth, double mLength){
        double rect_perimeter= (2.0 * mLength) + (2.0 * mWidth);
        return (rect_perimeter);
 }
 public static double rectangleArea(double mWidth, double mLength){
        double area = mWidth * mLength;
        return (area);
 }
 public static double sphereSurfaceArea(double mRadius){
        double sArea = 4.0 * Math.PI * mRadius * mRadius;
        return (sArea);
 }
public static double boxVolume(double mHeight, double mWidth, double mLength){
        double volume = mHeight * mWidth * mLength;
        return (volume);
 }
 public static double circleCircumference(double mRadius){
        double cCircumference = 2.0 * Math.PI * mRadius;
        return (cCircumference);
 }
 public static double circleArea(double mRadius){
        double cArea = Math.PI * ((mRadius * mRadius));
        return (cArea);
 }
 public static double rightTrianglePerimeter(double mLength, double mWidth){
        double rtPerimeter = mLength + mWidth + (Math.sqrt((Math.pow(mLength,2))+(Math.pow(mWidth,2))));
        return (rtPerimeter);
 }
 public static double rightTriangleArea(double mLength, double mWidth){
        double rtArea = mLength * mWidth / 2;
        return (rtArea);
 }
}
